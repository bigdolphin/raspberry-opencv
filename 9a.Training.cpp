...
opencv_traincascade -data xml/ -vec vector.vec -bg negative/bg.txt -numPos 200 -numNeg 200 -numStages 20 -acceptanceRatioBreakValue 0.004 -precalcValBufSize 300 -precalcIdxBufSize 300 -mode ALL -w 24 -h 24

Tri so acceptanceRatioBreakValue the hien so sai so chap nhan, phu thuoc vao so luong mau. VD: 200 mau, chap nhan sai 1 mau -> ty le 0.005
*/
...
int main(){
		...
		if(c=='c')
			...
			switch(c){
				...
				case 'p':
					...
					if((r.x-20>=0)&&(r.y-20>=0)
						&&(r.x+20<=frame.cols)&&(r.y+20<=frame.rows)){
	...
	snprintf(lineText,256,"opencv_traincascade -data ./data/xml/ -vec ./data/vector.vec -bg ./data/bg.txt -numPos %d -numNeg %d -numStages 20 -acceptanceRatioBreakValue %.5f -precalcValBufSize 300 -precalcIdxBufSize 300 -featureType LBP -minHitRate 0.999 -maxFalseAlarmRate 0.5 -w %.0f -h %.0f",count_pos,count_neg,acceptRate,w,h);
	printf("%s\r\n",lineText);
	system(lineText);
	printf("\r\n---------Done-----------\r\n");
	return 0;
